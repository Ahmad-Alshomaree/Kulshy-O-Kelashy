  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user) {
        ...
        if (!roleResponse.ok) {
          toast.error("Failed to set seller role. Please contact support.")
          setIsLoading(false)
          return
        }
      }

      toast.success("Seller account created successfully! Please sign in.")
      router.push("/login/seller?registered=true")
    } catch (err) {
